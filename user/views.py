...
def response(request, company_id):
    ...
    industry_max_value = ai.get_largest_enterprise_value()[0]
    ...
    port_congestion = ai.get_port_congestion_index()[0]
    logistics_iot = port_congestion
    credit_risk_scores = ai.get_credit_risk_score()[0]
    payment_performance_ownership_activity = ai.get_payment_performance_ownership_activity()
    payment_performance = payment_performance_ownership_activity[0]
    ownership = payment_performance_ownership_activity[1]
    supplier_health = algorithm.calc_supplier_health(credit_risk_scores=credit_risk_scores,
                                                     payment_performance=payment_performance,
                                                     ownership=ownership)
    trade_policy = ai.get_tarrifs_news()[0]
    geopolitical_conflict = ai.get_geopolitical_data()[0]
    ...
    analysis = ""  # TODO: Implementation by William
